// You'll just need to run: node convert-to-jest.js <optional path to specific test>
...
function convertToJest(filename) {
  ...
  fs.readFile(filename, 'utf8', function (err, data) {
    ...
    if (result.includes('createSpyObj')) {
      ...
      let numberOfSlashesinFilename = (filename.replace('./src/app/', '').match(/\//g) || []).length;
      let prefix = "./"
      for (let i = 0; i < numberOfSlashesinFilename; i++) {
        prefix += "../"
      }
